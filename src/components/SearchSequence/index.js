...
class SearchSequence extends Component {
  ...
  deleteLogo = id => {
    ...
    const list2 = initialHistoryList.filter(each => each.id !== id)
    this.setState({
      initialHistoryList: list2,
    })
  }
    ...
    return (
      <div className="bgContainer">
        <div className="bgContainer1">
          ...
          {list.length === 0 ? (
            <p className="one12">There is no history to show</p>
          ) : (
            <ul className="ul1">
              {list.map(each => (
                <SearchItem
